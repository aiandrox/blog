---
title: "【Rails】パーフェクトRuby on Rails 1, 2章"
...
categories:
  - Ruby, Rails
image: images/languages/perfect_rails.png
series:
  - 技術書
---

[![パーフェクトRuby on Rails](images/languages/perfect_rails.png)](https://www.amazon.co.jp/%E3%83%91%E3%83%BC%E3%83%95%E3%82%A7%E3%82%AF%E3%83%88-Ruby-Rails-%E3%80%90%E5%A2%97%E8%A3%9C%E6%94%B9%E8%A8%82%E7%89%88%E3%80%91-Perfect/dp/4297114623/ref=asc_df_4297114623/?tag=jpgo-22&linkCode=df0&hvadid=342520995287&hvpos=&hvnetw=g&hvrand=12362095176086800670&hvpone=&hvptwo=&hvqmt=&hvdev=c&hvdvcmdl=&hvlocint=&hvlocphy=1009300&hvtargid=pla-918633009109&psc=1&th=1&psc=1&tag=&ref=&adgrpid=69995157558&hvpone=&hvptwo=&hvadid=342520995287&hvpos=&hvnetw=g&hvrand=12362095176086800670&hvqmt=&hvdev=c&hvdvcmdl=&hvlocint=&hvlocphy=1009300&hvtargid=pla-918633009109)

Rails6.0 対応で、とにかく「めっちゃいい」らしい。ということで、読んでいく。
...
ここでは SQL のクエリと`ActiveRecord`の間。
...
`book.enum_column_before_type_cast`のように`_before_type_cast`を付けると、で DB に保存されている値（integer）を取得できる。

`Book.published`で where 絞り込み検索、`Book.not_published`のように not を付けると not 検索ができる。

enumerize という gem もあるのでオススメ
...
引数の数字を 3 桁ごとに`,`で区切った文字列を返す。  
delimiter で間の文字を指定することもできる（誰得？）。
...
クロスサイトスクリプティング（XXS）を防ぐために、ただの文字列は HTML にせずにエスケープするようになっている。
...
上記の処理によって`ActiveSupport::SafeBuffer`オブジェクトになり、js が実行される。  
